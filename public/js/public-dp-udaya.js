(function($) {
    ...
    $(document).ready(function() {
        if(verified_domain(api_key) == 1) {
            // $.getScript('https://maps.googleapis.com/maps/api/js?key=AIzaSyBpP-5WCGZu8_GxH6DdgqqUQFHtKprPHB0&libraries=places&callback=initMapUdaya', function() {
            ...
            //     });
            // }
            selected_location = $('#dapi-location-udaya').val();
            $('#dapi-location-udaya').on('focusout', function(){
                if($('#dapi-location-udaya').val() != ''){
                    setTimeout(function(){
                        lat = window.lat; 
                        lon = window.lon; 
            ...
            $('#wdate-udaya').val(get_current_date());

            $('.dapi-dbtns-udaya').on('click', function() {

                day_type = $(this).attr('dtyp');

                let widget_date = $('#wdate-udaya').val();
                ...
                $('#wdate-udaya').val(widget_date);
                widget_date_prev = widget_date;
                get_all_data(lat, lon, timezone);

            });

            $('#wdate-udaya').on('keyup change', function() {

                if(widget_date_prev != $('#wdate-udaya').val()){
                    get_all_data(lat, lon, timezone);
                }

            });

            $('#dapi-location-udaya').on('click', function(){
                selected_location = $('#dapi-location-udaya').val();

                $('#dapi-location-udaya').val('');

            });

            $('#dapi-location-udaya').on('focusout', function() {

                $('#dapi-location-udaya').val(selected_location);
    ...
    function get_all_data(lat, lon, timezone) {
        ...
        if(widget_year < 1422 || widget_year > 2823){
            ...
            //     return false;
            // }
            $('#wdate-udaya').val(temp_date);
            $('#dapi-wdate-udaya-error').hide();
