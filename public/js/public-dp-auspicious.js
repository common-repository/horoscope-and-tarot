(function($) {
    ...
    $(document).ready(function() {
        if(verified_domain(api_key) == 1) {
            // $.getScript('https://maps.googleapis.com/maps/api/js?key=AIzaSyBpP-5WCGZu8_GxH6DdgqqUQFHtKprPHB0&libraries=places&callback=initMapAuspi', function() {
            ...
            //     });
            // }
            selected_location = $('#dapi-location-auspi').val();
            $('#dapi-location-auspi').on('focusout', function(){
                if($('#dapi-location-auspi').val() != ''){
                    setTimeout(function(){
                        lat = window.lat; 
                        lon = window.lon; 
            ...
            $('#wdate-auspicious').val(get_current_date());

            $('.dapi-dbtns-auspicious').on('click', function() {

                day_type = $(this).attr('dtyp');

                let widget_date = $('#wdate-auspicious').val();
                ...
                $('#wdate-auspicious').val(widget_date);
                widget_date_prev = widget_date;
                get_all_data(lat, lon, timezone);

            });

            $('#wdate-auspicious').on('keyup change', function() {

                if(widget_date_prev != $('#wdate-auspicious').val()){
                    get_all_data(lat, lon, timezone);
                }

            });

            $('#dapi-location-auspi').on('click', function(){
                selected_location = $('#dapi-location-auspi').val();

                $('#dapi-location-auspi').val('');

            });

            $('#dapi-location-auspi').on('focusout', function() {

                $('#dapi-location-auspi').val(selected_location);
    ...
    function get_all_data(lat, lon, timezone) {
        ...
        if(widget_year < 1422 || widget_year > 2823){
            $('#dapi-wdate-auspicious-error').show();
            return false;
            ...
            //     return false;
            // }
            $('#wdate-auspicious').val(temp_date);
            $('#dapi-wdate-auspicious-error').hide();
