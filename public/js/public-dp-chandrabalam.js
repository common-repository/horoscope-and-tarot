(function($) {
    ...
    $(document).ready(function() {
        if(verified_domain(api_key) == 1) {
            // $.getScript('https://maps.googleapis.com/maps/api/js?key=AIzaSyBpP-5WCGZu8_GxH6DdgqqUQFHtKprPHB0&libraries=places&callback=initMapChandra', function() {
            ...
            //     });
            // }
            selected_location = $('#dapi-location-chandra').val();
            $('#dapi-location-chandra').on('focusout', function(){
                if($('#dapi-location-chandra').val() != ''){
                    setTimeout(function(){
                        lat = window.lat; 
                        lon = window.lon; 
            ...
            $('#wdate-chandra').val(get_current_date());

            $('.dapi-dbtns-chandra').on('click', function() {

                day_type = $(this).attr('dtyp');

                let widget_date = $('#wdate-chandra').val();
                ...
                $('#wdate-chandra').val(widget_date);
                widget_date_prev = widget_date;
                get_all_data(lat, lon, timezone);

            });

            $('#wdate-chandra').on('keyup change', function() {

                if(widget_date_prev != $('#wdate-chandra').val()){
                    get_all_data(lat, lon, timezone);
                }

            });

            $('#dapi-location-chandra').on('click', function(){
                selected_location = $('#dapi-location-chandra').val();

                $('#dapi-location-chandra').val('');

            });

            $('#dapi-location-chandra').on('focusout', function() {

                $('#dapi-location-chandra').val(selected_location);
    ...
    function get_all_data(lat, lon, timezone) {
        ...
        if(widget_year < 1422 || widget_year > 2823){
            ...
            //     return false;
            // }
            $('#wdate-chandra').val(temp_date);
            $('#dapi-wdate-chandra-error').hide();
