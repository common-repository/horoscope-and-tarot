;(function($) {
    ...
    var ch_key = atob(cyh_options.token); 
    var ch_tz = "";
    ...
    $( ".divine_custom_yearly_shortcode" ).each(function( index) {
        //ch_key = $(this).attr('key');
        ch_tz = $(this).attr('tz');

        ch_year = $(this).attr('year');
        ...
        $.ajax({
            ...
            method: 'post',
            async: false,
            data: {api_key: ch_key, sign: ch_sign, year: ch_year, timezone: ch_tz},
            success: function (data){
                ...
                if (response.success == 1) 
                {
                    ...
                    $(block).children( 'p.divine__custom__result' ).html('<b>'+capitalize(sign)+' '+capitalize(ch_category)+' Horoscope '+result.year+' </b><br>'+content);
