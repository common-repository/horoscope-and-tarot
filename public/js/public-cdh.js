;(function($) {
    ...
    var ch_key = atob(cdh_options.token); 
    var ch_tz = "";
    ...
    $( ".divine_custom_shortcode" ).each(function( index) {
        //ch_key = $(this).attr('key');
        ch_tz = $(this).attr('tz');

        ch_date = $(this).attr('date');
        ch_dt = $(this).attr('dt');
        ...
        $.ajax({
            ...
            method: 'post',
            async: false,
            data: {api_key: ch_key, sign: ch_sign, date: ch_date, timezone: ch_tz, ch_dt: ch_dt},
            success: function (data){
                ...
                if (response.success == 1) 
                {
                    ...
                    $(block).children( 'p.divine__custom__result' ).html('<b>'+capitalize(sign)+' '+capitalize(ch_category)+' Horoscope '+capitalize(ch_dt)+' </b><br>'+content);
