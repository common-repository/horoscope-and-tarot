(function($) {
    ...
    var auspi_timings_section_color = dp_options.auspi_timings_section_color;
    var auspi_timings_label_color = dp_options.auspi_timings_label_color;
    ...
    var chandra_and_tara_section_color = dp_options.chandra_and_tara_section_color;
    var chandra_and_tara_label_color = dp_options.chandra_and_tara_label_color;
    ...
    var panchak_and_udaya_section_color = dp_options.panchak_and_udaya_section_color;
    var panchak_and_udaya_label_color = dp_options.panchak_and_udaya_label_color;
    var show_sun_moon_section = dp_options.show_sun_moon_section;
    var show_panchang_section = dp_options.show_panchang_section;
    var show_lunar_and_samvat_section = dp_options.show_lunar_and_samvat_section;
    var show_rashi_and_nakshatra_section = dp_options.show_rashi_and_nakshatra_section;
    var show_ritu_and_ayana_section = dp_options.show_ritu_and_ayana_section;
    var show_auspi_timings_section = dp_options.show_auspi_timings_section;
    var show_in_auspi_timings_section = dp_options.show_in_auspi_timings_section;
    var show_nivas_and_shool_section = dp_options.show_nivas_and_shool_section;
    var show_chandra_and_tara_section = dp_options.show_chandra_and_tara_section;
    var show_other_calendar_and_epoch_section = dp_options.show_other_calendar_and_epoch_section;
    var show_panchak_and_udaya_section = dp_options.show_panchak_and_udaya_section;
    ...
    if (auspi_timings_section_color.length > 0) 
    {
        document.documentElement.style.setProperty('--auspi_timings_section_color', auspi_timings_section_color);
    }

    if (auspi_timings_label_color.length > 0) 
    {
        document.documentElement.style.setProperty('--auspi_timings_label_color', auspi_timings_label_color);
    }
    ...
    if (chandra_and_tara_section_color.length > 0) 
    {
        document.documentElement.style.setProperty('--chandra_and_tara_section_color', chandra_and_tara_section_color);
    }

    if (chandra_and_tara_label_color.length > 0) 
    {
        document.documentElement.style.setProperty('--chandra_and_tara_label_color', chandra_and_tara_label_color);
    }

    if (panchak_and_udaya_section_color.length > 0) 
    {
        document.documentElement.style.setProperty('--panchak_and_udaya_section_color', panchak_and_udaya_section_color);
    }

    if (panchak_and_udaya_label_color.length > 0) 
    {
        document.documentElement.style.setProperty('--panchak_and_udaya_label_color', panchak_and_udaya_label_color);
    }

    // if (dp_color_text.length > 0) 
    // {
    //     document.documentElement.style.setProperty('--cc-text-color', dp_color_text);
    // }
    // if (dp_font_size > 0) 
    // {
    //     document.documentElement.style.setProperty('--cc-font-size', dp_font_size+'px');
    // }
    // if (dp_color_theme.length > 0) 
    // {
    //     document.documentElement.style.setProperty('--cc-theme-color', dp_color_theme);
    // }
    // if (dp_color_category.length > 0) 
    // {
    //     document.documentElement.style.setProperty('--cc-category-color', dp_color_category);
    // }  

    $(document).ready(function() {
        if(verified_domain(api_key) == 1) {
            // $.getScript('https://maps.googleapis.com/maps/api/js?key=AIzaSyBpP-5WCGZu8_GxH6DdgqqUQFHtKprPHB0&libraries=places&callback=initMap', function() {
            ...
            $('#dapi-location').on('focusout', function(){
                if($('#dapi-location').val() != ''){
                    setTimeout(function(){
                        ...
                        console.log('dapi location', lat,lon, timezone);
                        get_all_data(lat, lon, timezone);
                    }, 600);
                }
            });

            $('#wdate').val(get_current_date());

            // var script = document.createElement("SCRIPT");
            
            // script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBpP-5WCGZu8_GxH6DdgqqUQFHtKprPHB0&libraries=places&callback=initMap';
            ...
            //         //console.log(results[0].location.lng());
            //         // console.log(results);
            //     }).catch((e) =>
            ...
            // $( "#dapi-location" ).autocomplete({
            //     source: function( request, response ) {
            //         $.ajax( {
            //             // url: 'http://localhost/dev-divine/widget/daily_panchang/widget-async.php',
            //             url: 'https://divineapi.com/widget/daily_panchang/widget-async.php',
            //             type: 'POST',
            //             dataType: "json",
            //             data: {srchtxt: $('#dapi-location').val(), reqtype: 'geoloc', wdate: $('#wdate').val()},
            ...
            //         // console.log( "Selected: " + ui.item.value + " aka " + ui.item.lat );
            //     }
            // });

            // $('#dapi-location').on('autocompleteselect', function (e, ui) {
            //     // console.log(ui);
            //     lat = ui.item.lat;
            //     lon = ui.item.lon;
            //     timezone = ui.item.tzone;
            ...
            //     // console.log(ui.item.lat +','+ ui.item.lon+','+ ui.item.tzone);
            //     // console.log(ui);
            //     // get_all_data(ui.item.lat, ui.item.lon, ui.item.tzone);

            //     $('#dapi-location').val(ui.item.label);
            //     selected_location = ui.item.label;
            //     get_all_data(lat, lon, timezone);
            // });                

            $('.dapi-dbtns').on('click', function() {

                day_type = $(this).attr('dtyp');

                let widget_date = $('#wdate').val();
                ...
                $('#wdate').val(widget_date);
                widget_date_prev = widget_date;
                get_all_data(lat, lon, timezone);

            });

            $('#wdate').on('keyup change', function() {

                if(widget_date_prev != $('#wdate').val()){
                    get_all_data(lat, lon, timezone);
                }
    ...
    function get_panchak_rahita_muhurta(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token) {

        try {

            $('#dapipanchrahita').html('<div class="row"><div class="col-lg-4 mx-auto text-center"><div class="dapi-sp sp-ldr11"></div></div></div>');

            $.ajax({
                url: 'https://astroapi-3.divineapi.com/indian-api/v1/panchak-rahita',//4
                method: 'post',
                headers: {
                    authorization: "Bearer " + access_token
                },
                ...
                success: function (data){
                    ...
                    if (response.success == 1) {
                        ...
                        let panchaka_rahita_data = response.data.panchak_rahita;

                        let panchaka_rahita_html = '';

                        $.each(panchaka_rahita_data, function(key, val) {

                            let rahita_name = val.panchak;
                            if (rahita_name.indexOf('Good') !== -1) {
                                rahita_name = '<span class="dapi-text-green">Good Muhurta</span>'
                            }

                            let rahita_starts_at = convert_date_to_hours_mins_ampm(val.start_time);
                            let rahita_start = convert_date_in_d_m_y_format(val.start_time)
                            let rahita_start_content = convert_date_in_d_M_format(val.start_time);
                            let rahita_start_date = (selected_date != rahita_start && rahita_start.length > 0) ? ', ' + rahita_start_content : '';
                            let rahita_ends_at = convert_date_to_hours_mins_ampm(val.end_time);
                            let rahita_end = convert_date_in_d_m_y_format(val.end_time)
                            let rahita_end_content = convert_date_in_d_M_format(val.end_time);
                            let rahita_end_date = (selected_date != rahita_end && rahita_end.length > 0) ? ', ' + rahita_end_content : '';
                            panchaka_rahita_html += '<p class="dapi-p">'+rahita_name+' - '+rahita_starts_at+rahita_start_date+' to '+rahita_ends_at+rahita_end_date+'</p>';

                        });

                        $('#dapipanchrahita').html(panchaka_rahita_html);
                        $('#panchakaudaya').show();

                    } else {
                        $('#dapipanchrahita').html('<p class="text-danger">Something went wrong while fetching panchaka rahita muhurta details!</p>');
                    }
            ...
            console.log('Error in get panchak rahita muhurta: ' + e);

        }

    }

    function get_udaya_lagna(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token) {

        try {

            $('#dapiudayalgn').html('<div class="row"><div class="col-lg-4 mx-auto text-center"><div class="dapi-sp sp-ldr11"></div></div></div>');

            $.ajax({
                url: 'https://astroapi-3.divineapi.com/indian-api/v1/uday-lagna',//4
                method: 'post',
                headers: {
                    authorization: "Bearer " + access_token
                },
                ...
                success: function (data){
                    ...
                    if (response.success == 1) {
                        ...
                        let udaya_lagana_data = response.data.uday_lagna;

                        let udaya_lagana_html = '';

                        let signs_array = new Array();
                        signs_array['Mithuna'] = 'gemini';
                        signs_array['Karka'] = 'cancer';
                        signs_array['Simha'] = 'leo';
                        signs_array['Kanya'] = 'virgo';
                        signs_array['Tula'] = 'libra';
                        signs_array['Vrishchika'] = 'scorpio';
                        signs_array['Dhanu'] = 'sagittarius';
                        signs_array['Makara'] = 'capricorn';
                        signs_array['Kumbha'] = 'aquarius';
                        signs_array['Meena'] = 'pisces';
                        signs_array['Mesha'] = 'aries';
                        signs_array['Vrishabha'] = 'taurus';


                        $.each(udaya_lagana_data, function(key, val) {

                            let udaya_name = val.sign_hi;
                            let img_sign = udaya_name.split(' - ');

                            let udaya_starts_at = convert_date_to_hours_mins_ampm(val.start_time);
                            let udaya_start = convert_date_in_d_m_y_format(val.start_time)
                            let udaya_start_content = convert_date_in_d_M_format(val.start_time);
                            let udaya_start_date = (selected_date != udaya_start && udaya_start.length > 0) ? ', ' + udaya_start_content : '';
                            let udaya_ends_at = convert_date_to_hours_mins_ampm(val.end_time);
                            let udaya_end = convert_date_in_d_m_y_format(val.end_time)
                            let udaya_end_content = convert_date_in_d_M_format(val.end_time);
                            let udaya_end_date = (selected_date != udaya_end && udaya_end.length > 0) ? ', ' + udaya_end_content : '';
                            udaya_lagana_html += '<p class="dapi-p"><img class="dapi_img" src="'+plgn_base_url+'public/images/panchang/signs/'+signs_array[img_sign[1]]+'.svg"/>'+udaya_name+' - '+udaya_starts_at+udaya_start_date+' to '+udaya_ends_at+udaya_end_date+'</p>';

                        });

                        $('#dapiudayalgn').html(udaya_lagana_html);
                        $('#panchakaudaya').show();

                    } else {
                        $('#dapiudayalgn').html('<p class="text-danger">Something went wrong while fetching udaya lagna details!</p>');
                    }
    ...
    // function get_city_coordinates(srchtxt){
    // 	try {
    // 		$( "#dapi-location2" ).autocomplete({
    // 			source: function( request, response ) {
    // 				$.ajax( {
    // 					url: 'http://localhost/dev-divine/widget/daily_panchang/widget-async.php',
    // 					type: 'POST',
    // 					dataType: "json",
    // 					data: {srchtxt: $('#dapi-location2').val(), reqtype: 'geoloc', wdate: '2023-06-21'},
    // 					success: function( data ) {
    // 						console.log(data.search_data);
    // 						response( data.search_data );
    // 					}
    ...
    // 		console.log('Error in get city co-ordinates: ' + e);

    // 	}
    // }

    // function get_city_coordinates_old(srchtxt) {

    //     try {

    //         if (srchtxt.length >= 2) {
                
    //             // if ($('#dapi-location').data('ui-autocomplete') != 'undefined') {

    //             //     $('#dapi-location').autocomplete("destroy");
    //             // }
    // 			$( "#dapi-location" ).autocomplete({
    // 				source: [],
    // 				select: function (event, ui) {
    // 					var v = ui.item.hdn;
    // 					let lat_long = v.split(" ");
    // 					lat = lat_long[0];
    // 					lon = lat_long[1];
    // 					timezone = lat_long[2];
    // 					// get_all_data(lat_long[0], lat_long[1], lat_long[2]);
    // 					return false;
    // 				},
    // 				appendTo: ".dapi-location",
    // 			});
    ...
    //                 url: 'http://localhost/dev-divine/widget/daily_panchang/widget-async.php',
    //                 method: 'post',
    //                 data: {srchtxt: srchtxt, reqtype: 'geoloc', wdate: wdate},
    //                 // beforeSend: function(){
    //                 //     if ($('#dapi-location').data('ui-autocomplete') != 'undefined') {
    ...
    //                         let source = $.map(search_data, function (value, key) { 
    //                             return {
    //                                 label: value.city,
    //                                 value: value.city,
    //                                 hdn: parseFloat(value.lat).toFixed(4) + ' ' + parseFloat(value.lon).toFixed(4) + ' ' + parseFloat(value.tzone).toFixed(1),
    // 								lat: parseFloat(value.lat).toFixed(4),
    // 								lon: parseFloat(value.lon).toFixed(4),
    // 								tzone: parseFloat(value.tzone).toFixed(4),
    //                             };
    //                         });
    ...
    //                             source: source,
    // 							// lookup: source,
    // 							// serviceUrl: 'http://localhost/dev-divine/widget/daily_panchang/widget-async.php',
    // 							// minLength: 0,
    //                             select: function (event, ui) {
    //                                 var v = ui.item.hdn;
    //                                 let lat_long = v.split(" ");
    ...
    // 						// 	return $( "<li>" )
    // 						// 	.append( "<div>" + ul.item.timezone + '<br>' + item.label + "</div>" )
    // 						// 	.appendTo( ul );
    // 						// };
    ...
    // 						// 	id += 1;
    // 						// 	list += '<li class="ui-menu-item"><div id="ui-id-'+id+'" tabindex="-1" class="ui-menu-item-wrapper" value='+ source[key].label +' lat='+ source[key].lat +' lon='+ source[key].lon +' value='+ source[key].label +'>' + source[key].label + "</div></li>";
    // 						// 	$('#ui-id-1').show();
    // 						// 	$( "#dapi-location" ).autocomplete({
    // 						// 		source: source,
    // 						// 		select: function (event, ui) {
    ...
    // function strtotime(string_to_convert) {

    //     var dateComponents = string_to_convert.split("/");
        
    //     let date_string = new Date(
    //         parseInt(dateComponents[0]),
    //         parseInt(dateComponents[1]) - 1, // Month is zero-based
    //         parseInt(dateComponents[2]),
    //     );

    //     return date_string.getTime();
    ...
    function get_all_data(lat, lon, timezone) {
        ...
        if (show_sun_moon_section == 'on') {

            get_sunrise_and_moonrise(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_panchang_section == 'on' || show_rashi_and_nakshatra_section == 'on') {

            get_panchang(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_lunar_and_samvat_section == 'on') {

            get_lunar_month_and_samvat(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_ritu_and_ayana_section == 'on') {

            get_ritu_and_ayana(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_auspi_timings_section == 'on') {

            get_auspicious_timings(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_in_auspi_timings_section == 'on') {

            get_inauspicious_timings(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_nivas_and_shool_section == 'on') {

            get_nivas_and_shool(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_chandra_and_tara_section == 'on') {

            get_chandrabalam_tarabalam(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_other_calendar_and_epoch_section == 'on') {

            get_calendar_and_epoch(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

        }

        if (show_panchak_and_udaya_section == 'on') {

            get_panchak_rahita_muhurta(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);

            get_udaya_lagna(api_key, widget_day, widget_month, widget_year, timezone, lat, lon, access_token);
