;(function($) {
    ...
    var ch_key = atob(cmh_options.token); 
    var ch_tz = "";
    ...
    $( ".divine_custom_monthly_shortcode" ).each(function( index) {
        //ch_key = $(this).attr('key');
        ch_tz = $(this).attr('tz');

        ch_month = $(this).attr('month');
        ...
        $.ajax({
            ...
            method: 'post',
            async: false,
            data: {api_key: ch_key, sign: ch_sign, month: ch_month, timezone: ch_tz},
            success: function (data){
                ...
                if (response.success == 1) 
                {
                    ...
                    $(block).children( 'p.divine__custom__result' ).html('<b>'+capitalize(sign)+' '+capitalize(ch_category)+' Horoscope '+result.month+' </b><br>'+content);
